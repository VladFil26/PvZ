					...
					public void actionPerformed(ActionEvent e) {
						...
						if (y != board[0].length - 1) {
							...
							case 0:
								...
								if (board[x][y + 1].getIcon() == imageTile
										|| board[x][y + 1].getIcon() == imageZombie) {
				...
				if (num % 10 == 0) {
					...
					for (int i = 4; i >= 0; i--) {
						for (int j = 8; j >= 0; j--) {
							...
							/*
							 * if (bulletArray[i][j] == 0) { board[i][j].setIcon(imageTile);
							 * 
							 * }else if(bulletArray[i][j] == 1){ board[i][j].setIcon(imagePea); }
							 */
